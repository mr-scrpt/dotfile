return {
  {
    ...
    opts = function(_, opts)
      ...
      -- 1. НАСТРОЙКА ОТОБРАЖЕНИЯ (ЧИСТЫЙ ЭКРАН)
      opts.diagnostics = opts.diagnostics or {}

      -- Отключаем текст справа от кода (Error: ...)
      opts.diagnostics.virtual_text = false
      -- Отключаем текст под кодом (если есть lsp_lines)
      opts.diagnostics.virtual_lines = false
      -- Оставляем подчеркивание (волнистая линия), чтобы видеть где ошибка
      opts.diagnostics.underline = true
      -- Оставляем значки слева (красные точки/крестики)
      opts.diagnostics.signs = true
      -- Не обновлять ошибки пока печатаешь (меньше шума)
      opts.diagnostics.update_in_insert = false

      opts.diagnostics.float = {
        border = "rounded",
        source = "always", -- Показывать источник ошибки (например, eslint)
        header = "",
        prefix = "",
      }
      ...
      vim.list_extend(keys, {
        -- [[ ВАЖНОЕ ИЗМЕНЕНИЕ ЗДЕСЬ ]]
        {
          ...
          desc = "Показать ошибку (Float)",
        },
        -- Если нужна обычная документация (hover), перенеси её на gK или оставь <leader>ck
        { "<leader>ck", vim.lsp.buf.hover, desc = "Hover Documentation" },

        -- Твои остальные биндинги
        {
          "<leader>co",
          function()
            LazyVim.lsp.execute({
      ...
      -- [[ ЛОГИКА С АВТО-ОТКЛЮЧЕНИЕМ ]]
      -- Твой код без изменений, он хороший
      ...
      vim.list_extend(keys, {
        ...
        },
        {
          ...
          function()
            ...
            -- Проверяем включены ли линии вообще
            if
