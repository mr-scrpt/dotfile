...
START_PROMPT = (
    ...
    "тема → сессия → тип поиска → бриф → источники → критерии (покажи их мне до поиска) → "
    "планы по источникам → предложения → отзывы → отчёт. "
    "Все списки — через shop_menu; свободные поля спрашивай по одному. "
    "Критерии пиши только по данным `observed`, не придумывай параметры. Начинай."
)
